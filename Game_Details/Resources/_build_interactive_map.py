...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .side-section { padding:15px 16px; border-bottom:1px solid var(--border-soft); }
  .side-section h3 { color:#c9d4dc; font-size:10px; font-weight:800; letter-spacing:.13em; margin-bottom:10px; }
...
svg.addEventListener('wheel', (ev) => {
  ...
  // Small per-tick factor (was 1.15 - far too large a jump per scroll notch, per user
  // report). Max zoom-out is capped at vb.w (the dynamically-fitted default view from
  // Task 3, top/bottom-flush against #mapwrap) rather than the old vb.w*3, so the wheel
  // can never pull the map out past its already-optimal fitted view into empty space.
