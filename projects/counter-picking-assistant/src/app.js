import { LANES, TROOPS, combatExplorerQuery, recommendFormations } from "./engine.mjs?v=counter-3";
...
function render() {
  ...
  document.querySelector("#summary").textContent = `${result.threats.length} explicit threats · ${owned.length} owned dragons`;
...
function recommendationCard(item, index, enemies) {
  ...
  return `<article class="recommendation"><div class="rec-head"><div><p class="eyebrow">${index === 0 ? "BEST COVERAGE" : index === 1 ? "ALTERNATIVE" : "SECOND ALTERNATIVE"}</p><h2>${item.formation.map(pick => esc(pick.dragon.name)).join(" · ")}</h2></div><span class="coverage">${Math.round(item.coverage * 100)}% threat coverage</span></div><div class="formation">${item.formation.map(pick => `<div class="pick"><small>${esc(pick.lane)}</small><h3><a href="${dragonLink(pick.dragon.slug)}">${esc(pick.dragon.name)}</a></h3><span>${esc(pick.dragon.breed)} · L${pick.progression.level} · ${pick.progression.stars}★ · Habits ${esc(habitSummary(pick.dragon, pick.progression))}</span>${pick.counters.length ? `<ul class="evidence">${pick.counters.map(entry => `<li><b>${esc(entry.reason)}</b> via ${esc(entry.ability)}<q>${esc(entry.exact_text)}</q></li>`).join("")}</ul>` : `<p class="muted">Adds formation depth and ${pick.controls.length ? `${esc(pick.controls.map(control => control.name).join("/"))} control pressure` : "shared-troop compatibility"}; no direct counter claim.</p>`}</div>`).join("")}</div><div class="troop"><b>Shared troop: ${esc(item.troop.troop)}</b> — ${esc(troopNotes)}${uncovered.length ? `<br><b>What can go wrong:</b> No explicit answer found for ${esc(uncovered.map(threat => threat.name).join(", "))}.` : `<br>Every extracted threat has at least one explicit answer in this formation.`}</div><div class="actions"><a href="../combat/?${combatExplorerQuery(item, enemies, item.troop.troop)}">Inspect in Combat Explorer ↗</a></div></article>`;
}
