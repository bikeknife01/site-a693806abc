...
| `rarity_label` | Displayed or explicitly user-confirmed rarity. Never infer a missing rarity without confirmation. |
...
| `derived.preferred_troops` | Troop types supported by positive affinity. Still test opposing troop matchups. |
...
- A formation has `left_flank`, `vanguard`, and `right_flank`. “Same lane” targets the opposing unit in the dragon's lane. “Within adjacency” is not interchangeable with “any lane”; preserve the displayed scope.
...
- Positive affinity is the source-backed default for `preferred_troops`; negative affinity belongs in avoidance. Separately account for the troop cycle: Cavalry > Shieldbearers > Archers > Spearmen > Cavalry. Siege is disadvantaged against all combat troop types and specializes in POI Durability damage.
...
The live game has two official `Shadow of the Greens` additions not yet represented by
screenshot-complete records: Starshower and Vermithor. `data/live_updates.json`
captures their official profile summaries and affinities. Those profiles omit exact
numeric scaling, Habit upgrade tracks, explicit Star gates, and Vanguard text, so use
them only for conditional strategy and disclose the missing evidence. They also add
Solar Flare and Protect, defined in `data/effects.json`.
...
- Cite positive/negative affinity fields for troop assignments and separately cite `data/game_mechanics.json` when using troop matchup advantage.
- Check `data/live_updates.json` when a named dragon has no record under `data/dragons/`.
  Never infer missing Star gates, values, or Vanguard effects from an official profile summary.
